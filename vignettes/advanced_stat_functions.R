...
## ---- echo=FALSE, warning=FALSE, message=FALSE, error=FALSE,cache=TRUE, label="F2-13", fig.margin = TRUE, fig.cap="Confidence intervals for the mean number of units consumed by each actor.", fig.width=4, fig.height=4----
ggplot(bond, aes(x = Actor, y = Alcohol_Units)) +
    stat_summary(fun.ymin = function(i) mean(i) - std_err(i),
                 fun.ymax = function(i) mean(i) + std_err(i),
                 colour = "steelblue", geom = "errorbar",
